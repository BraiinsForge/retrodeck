...
(defun draw-dashboard-bitcoin-icon (x y width height color)
  (let ((center-x (+ x (floor width 2)))
        (center-y (+ y (floor height 2)))
        (symbol-color (dashboard-color :white)))
    (dolist (rect `((,(- center-x 48) ,(- center-y 72) 96 12)
                    (,(- center-x 60) ,(- center-y 60) 120 12)
                    (,(- center-x 72) ,(- center-y 48) 144 96)
                    (,(- center-x 60) ,(+ center-y 48) 120 12)
                    (,(- center-x 48) ,(+ center-y 60) 96 12)))
      (apply #'fill-canvas-rect (append rect (list color))))
    (fill-canvas-rect (- center-x 22) (- center-y 58) 6 116 symbol-color)
    (fill-canvas-rect (- center-x 6) (- center-y 58) 6 116 symbol-color)
    (draw-centered-text (- center-x 55) (- center-y 56) 110 112
                        "B" 14 symbol-color)))
...
(defun draw-dashboard-compact-logo (x y width height game)
  ...
  (let ((id (getf game :id))
    ...
    (cond
      ...
      ((dashboard-built-in-game-p game "node-mode")
       (draw-dashboard-bitcoin-icon x y width height color))
